...
async def join_voice_channel(guild: Guild, channel: VoiceChannel):
    if guild.id in voice_clients:
        await voice_clients[guild.id].move_to(channel)
    else:
        voice_clients[guild.id] = await channel.connect()

async def leave_voice_channel(guild: Guild):
    if guild.id in voice_clients:
...
async def join(ctx, channel_name: str = None):
    ...
    if channel:
        await join_voice_channel(ctx.guild, channel)
    else:
